...
( async () => {
    ...
    AfterAsyncTasks( response.data );

} )();


// 비동기 함수가 완료된 후 실행할 코드

function AfterAsyncTasks( responseData ) {
    /* -- -- -- -- 제품을 선택하세요 -- -- -- -- */
    ...
    const colorArr = responseData.color;
    ...
    document.querySelectorAll('.inner-list').forEach( (v,i) => {
        ...
        if ( colorNum > 1 ) {
            ...
            span2.innerHTML = `${responseData.title} / ${clr_name(colorName)}`;
        }
        else {
            span2 = document.createElement('span');
            span2.classList.add('submenu-txt');
            span2.classList.add('pad');
            span2.innerHTML = `제품명: ${responseData.title}`;
        }
    ...
    document.querySelectorAll('.inner-list').forEach( (v,i) => {
        ...
        v.addEventListener( 'click', e => {
            ...
            const createBox = () => {
                ...
                div7.innerHTML = `${responseData.price.toLocaleString()}원`;
